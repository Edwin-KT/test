...
void RouteHandler::setupRoutes(crow::SimpleApp& app) {
    ...
    CROW_ROUTE(app, "/leaveGame").methods("POST"_method)(RouteHandler::handleLeaveGame);
...
crow::response RouteHandler::handleSendMessage(const crow::request& req, int gameId) {
    try {
        ...
        std::regex filterPattern(
            R"((\b\d+\b)|(\b(unu|doi|trei|patru|cinci|sase|
                             sapte|opt|noua|zece|unspre|doispre|
                             treispre|paispre|cincispre|saispre|
                             saptespre|optspre|nouaspre|douaz|
                             treiz|patruz|cinciz|saiz|saptez|
                             optz|nouaz|suta)\w*))",
