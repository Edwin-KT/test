...
crow::response handleGetGameState(const crow::request& req, int gameId) {
    ...
    if (!session->started || !session->gameLogic) {
        // Return a clean JSON object for the Lobby state
        nlohmann::json j;
        j["status"] = "Lobby"; // Client checks this string
    ...
    // Convert to simple state expected by Client
    json simpleState;
    ...
    // Locate my hand in the players array
